...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if not testing:
            self.train_trial += 1
            self.epsilon = math.pow(self.a, self.train_trial)
            # self.epsilon = 1.0 / (self.train_trial ** 2)
            # self.epsilon = math.exp(-self.a * self.train_trial)
            # self.epsilon = math.cos(self.a * self.train_trial)
        else:
            self.epsilon = 0.0
            self.alpha = 0.0
    ...
    def get_maxQ(self, state):
        ...
        state_Q = self.Q.get(state)
        maxQ = max(state_Q.values())
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            ...
            if temp <= self.epsilon:
                ...
                m = max(self.Q[state].values())
                actions = [k for k, v in self.Q[state].items() if v == m]
    ...
    def learn(self, state, action, reward):
        ...
        self.Q[state][action] = (1 - self.alpha) * Q_s_a + self.alpha * reward
